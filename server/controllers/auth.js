...
export const getSubscriptionStatus = async (req, res) => {
  ...
  const viewerId = req.userId || req.query.userId;
  ...
  try {
    const subscriberCount = await users.countDocuments({ subscribedChannels: channelName });
    let subscribed = false;

    if (viewerId && mongoose.Types.ObjectId.isValid(viewerId)) {
      const viewer = await users.findById(viewerId).select("subscribedChannels");
      subscribed = Boolean(viewer?.subscribedChannels?.includes(channelName));
    }

    return res.status(200).json({ channelName, subscribed, subscriberCount });
  } catch (error) {
    console.error("Subscription status error:", error);
    return res.status(500).json({ message: "Unable to fetch subscription status." });
  }
};

export const toggleSubscription = async (req, res) => {
  const viewerId = req.userId || req.body.userId;
  ...
  try {
    ...
    const currentSubscriptions = new Set(viewer.subscribedChannels || []);
    let subscribed;

    if (currentSubscriptions.has(channelName)) {
      currentSubscriptions.delete(channelName);
      subscribed = false;
    } else {
      currentSubscriptions.add(channelName);
      subscribed = true;
    }

    viewer.subscribedChannels = [...currentSubscriptions];
    await viewer.save();
